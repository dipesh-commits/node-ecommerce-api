...
//        id=new MongoId
   ...
   var newvalues = {$set: {'local.age':req.body.age,
                            'local.profile_picture': req.files[0].path,
...
// res.send('fuck you man ');
   ...
   forgot: async(req, res, next) => {
         const {email} = req.body;
          ...
          var token2 = crypto.randomBytes(2);
...
//        id=new MongoId
  ...
  /*changepassword: async(req, res, next)=>{
     ...
     const user = await User.findOne({"local.password": passwordHash});
        // res.send('fuck you man ');
          ...
          const hashedpassword = await bcrypt.hash(newPassword,genSalt(10));
           console.log(hashedpassword);
            MongoClient.connect(url, { useNewUrlParser: true }, function(err, db) {
            if (err) throw err;
            var dbo = db.db("1");
    //        id=new MongoId
            var myquery = {'local.password':passwordHash};

            var newvalues = {$set: {'local.password':hashedpassword}};
            dbo.collection("users").updateOne(myquery, newvalues, function(err, res) {
              if (err) throw err;
              console.log(res.result.nModified + " document(s) updated");
              db.close();
